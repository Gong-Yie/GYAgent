...
class FileProvenanceStore:
    ...
    def known_mind_ids(self) -> tuple[str, ...]:
        ...
        if not isinstance(raw, list) or any(not isinstance(item, str) for item in raw):
            raise ValueError("provenance manifest mind_ids must be a string array")
    ...
    def _read_manifest(self) -> dict[str, object] | None:
        ...
        if value.get("schema_version") != MANIFEST_SCHEMA_VERSION:
            raise ValueError("provenance manifest schema version is invalid")
        return value
